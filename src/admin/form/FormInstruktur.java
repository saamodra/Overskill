...
public class FormInstruktur extends javax.swing.JFrame {
    ...
    private void updateData() {
            ...
            try {
                ...
                if(txtPassword.getText().equals("")) {
                    query = "UPDATE Instruktur SET Nama_Instruktur=?,Tanggal_lahir=?, Jenis_Kelamin=?, Alamat=?, No_telp=?, Email=?, "
                            + "Pengalaman_kerja=?, Username=? "
                            + "WHERE ID_Instruktur = ?";
                } else {
                    query = "UPDATE Instruktur SET Nama_Instruktur=?,Tanggal_lahir=?, Jenis_Kelamin=?, Alamat=?, No_telp=?, Email=?, "
                            + "Pengalaman_kerja=?, Username=?, Password=? "
                            + "WHERE ID_Instruktur = ?";
                    updatePass = true;
                }
    ...
    private boolean validateAll() {
        ...
        if(formtype.equals("Tambah")){
            ...
            return nama && tglLahir && jenkel && alamat && notelp && email && pk && username && passlama;
        } else {
            ...
            if(!txtPassword.getText().equals("")) {
                ...
                return nama && tglLahir && jenkel && alamat && notelp && email && pk && username && passlama && passbaru;
            } else {
                return nama && tglLahir && jenkel && alamat && notelp && email && pk && username;
            }
        }
    ...
    private void initComponents() {
        ...
        txtPengalaman = new org.jdesktop.swingx.JXTextArea();
        ...
        txtPengalaman.setLineWrap(true);
        txtPengalaman.setRows(5);
        txtPengalaman.setWrapStyleWord(true);
        txtPengalaman.setFont(new java.awt.Font("Segoe UI", 0, 12)); // NOI18N
        txtPengalaman.setPrompt("Pengalaman Kerja");
        jScrollPane2.setViewportView(txtPengalaman);

        jPanel4.add(jScrollPane2, new org.netbeans.lib.awtextra.AbsoluteConstraints(580, 40, 240, 120));

        txtUsername.setFont(new java.awt.Font("Segoe UI", 0, 12)); // NOI18N
        txtUsername.setPrompt("Username");
        jPanel4.add(txtUsername, new org.netbeans.lib.awtextra.AbsoluteConstraints(580, 185, 240, 30));
        ...
        jPanel4.add(btnSimpan, new org.netbeans.lib.awtextra.AbsoluteConstraints(730, 360, 90, 30));

        txtPassword.setFont(new java.awt.Font("Segoe UI", 0, 12)); // NOI18N
        jPanel4.add(txtPassword, new org.netbeans.lib.awtextra.AbsoluteConstraints(580, 240, 240, 30));

        txtPassword1.setFont(new java.awt.Font("Segoe UI", 0, 12)); // NOI18N
        jPanel4.add(txtPassword1, new org.netbeans.lib.awtextra.AbsoluteConstraints(580, 295, 240, 30));
    ...
    private org.jdesktop.swingx.JXTextArea txtPengalaman;
